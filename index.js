...
export default {
  async scheduled(event, env, ctx) {
    async function getLatestTopic() {
      ...
      try {
        ...
        return all.length ? all[Math.floor(Math.random() * Math.min(all.length, 3))] : null;
      } catch (e) { return null; }
    }
    ...
    const prompts = [
      'أنت محرر صحفي متخصص في الشأن المغربي. اكتب مقدمة احترافية وجذابة من 5 أسطر عن: "' + topic + '". اشرح أهميته للمواطن المغربي. أسلوب إنساني طبيعي ومبسط.',
    ...
    for (let i = 0; i < prompts.length; i++) {
      try {
        const r = await env.AI.run("@cf/meta/llama-3-8b-instruct", { prompt: prompts[i], max_tokens: 1500, temperature: 0.7 });
    ...
    try {
      const imgMap = {
        "الدعم والحماية الاجتماعية": "Professional flat design, digital social services concept, smartphone with form, shield icon, green blue colors, geometric shapes, clean corporate style, 4k, NO humans NO animals NO faces",
        "مباريات التوظيف والتدريب": "Professional flat design, career education concept, graduation cap laptop, certificates books, blue purple colors, geometric patterns, clean vector, 4k, NO humans NO animals NO faces",
        "الوثائق الإدارية والتأشيرات": "Professional flat design, official documents passport stamps, globe routes, official seals, amber navy colors, geometric design, 4k, NO humans NO animals NO faces",
        "المقاول الذاتي والعمل الحر": "Professional flat design, business startup concept, rocket laptop, growth charts, coins lightbulb, green blue colors, geometric background, 4k, NO humans NO animals NO faces",
        "مستجدات وأخبار المغرب": "Professional flat design, Morocco digital news concept, smartphone notifications, golden blue colors, abstract geometric background, 4k, NO humans NO animals NO faces"
      };
    ...
    if (!idx.some(x => x.title === topic)) {
      idx.unshift({ id, slug, title: topic, description: desc, date: post.date, dateISO: post.dateISO, category: cat.name, categoryColor: cat.color, categoryIcon: cat.icon, readTime: post.readTime, views: post.views, hasImage: !!img64 });
      ...
      await Promise.allSettled([
        ...
        fetch("https://api.indexnow.org/indexnow", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ host: new URL(site).hostname, key: env.INDEXNOW_KEY || "key", urlList: [site + "/post/" + slug, site + "/"] }) }).catch(() => {})
      ]);
    }
  },
  ...
  async fetch(request, env) {
    ...
    const H = (html, age = 300) => new Response(html, { headers: { "Content-Type": "text/html;charset=UTF-8", "Cache-Control": "public,max-age=" + age, "X-Content-Type-Options": "nosniff", "X-Frame-Options": "SAMEORIGIN", "Referrer-Policy": "strict-origin-when-cross-origin" } });
    ...
    const staticPages = { "/privacy": pgPrivacy, "/terms": pgTerms, "/contact": pgContact, "/about": pgAbout, "/dmca": pgDMCA, "/disclaimer": pgDisclaimer };
    if (staticPages[path]) { const idx = await getIdx(); return H(staticPages[path](base, idx), 86400); }
    ...
    if (path.startsWith("/post/")) {
      ...
      if (p) {
        ...
        try { await Promise.all([env.BLOG_KV.put("post:" + slug, JSON.stringify(p), { expirationTtl: 7776000 }), env.BLOG_KV.put("posts_index", JSON.stringify(idx))]); } catch (e) {}
...
const catColor = n => (CATS.find(c => c.name === n) || {}).color || "#0f4c81";
const catIcon = n => (CATS.find(c => c.name === n) || {}).icon || "📰";
...
function mkSitemap(idx, base) {
  ...
  statics.forEach(s => { x += `<url><loc>${base}${s}</loc><lastmod>${now}</lastmod><changefreq>${s === "/" ? "hourly" : "monthly"}</changefreq><priority>${s === "/" ? "1.0" : "0.4"}</priority></url>\n`; });
  cats.forEach(c => { x += `<url><loc>${base}/category/${encodeURIComponent(c)}</loc><lastmod>${now}</lastmod><changefreq>daily</changefreq><priority>0.7</priority></url>\n`; });
  idx.forEach(p => { x += `<url><loc>${base}/post/${p.slug}</loc><lastmod>${p.dateISO || now}</lastmod><changefreq>weekly</changefreq><priority>0.9</priority>${p.hasImage ? `<image:image><image:loc>${base}/image/${p.slug}</image:loc><image:title>${esc(p.title)}</image:title></image:image>` : ""}</url>\n`; });
  return x + "</urlset>";
}
...
function mkRSS(idx, base) {
  ...
  idx.slice(0, 20).forEach(p => { x += `<item><title>${esc(p.title)}</title><link>${base}/post/${p.slug}</link><description><![CDATA[${p.description || ""}]]></description><pubDate>${new Date(p.dateISO || Date.now()).toUTCString()}</pubDate><category>${esc(p.category)}</category><guid isPermaLink="true">${base}/post/${p.slug}</guid></item>\n`; });
  return x + "</channel>\n</rss>";
}
...
:root{--p:#0f4c81;--pl:#1a6fb5;--pd:#0a3459;--s:#e8b100;--sl:#ffd633;--d:#1a1a2e;--g50:#f8fafc;--g100:#f1f5f9;--g200:#e2e8f0;--g300:#cbd5e1;--g400:#94a3b8;--g500:#64748b;--g600:#475569;--g700:#334155;--w:#fff;--sh:0 2px 8px rgba(0,0,0,.08);--shl:0 8px 24px rgba(0,0,0,.12);--r:12px;--rl:16px;--t:all .3s ease}
html{scroll-behavior:smooth;font-size:16px}
body{font-family:'Cairo',sans-serif;background:var(--g50);color:var(--d);line-height:1.8;direction:rtl;text-align:right;overflow-x:hidden;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;min-height:100vh;width:100%}
...
.hd-in{max-width:1200px;margin:0 auto;padding:10px 16px;display:flex;justify-content:space-between;align-items:center;gap:12px}
.logo{display:flex;align-items:center;gap:10px;flex:1;min-width:0}
.logo-i{width:44px;height:44px;min-width:44px;background:linear-gradient(135deg,var(--p),var(--pl));border-radius:10px;display:flex;align-items:center;justify-content:center}
.logo-i svg{width:24px;height:24px;fill:var(--w)}
.logo-t h1{font-size:17px;font-weight:800;color:var(--p);line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.logo-t span{color:var(--s)}
.logo-t p{font-size:10px;color:var(--g500);white-space:nowrap}

/* HAMBURGER */
.mbtn{display:none;background:var(--w);border:2px solid var(--g300);cursor:pointer;padding:0;border-radius:8px;width:40px;height:40px;flex-direction:column;align-items:center;justify-content:center;gap:5px;flex-shrink:0}
...
.mbtn.open i:nth-child(2){opacity:0}
.mbtn.open i:nth-child(3){transform:translateY(-7px) rotate(-45deg)}

/* OVERLAY */
.ov{display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:999;-webkit-backdrop-filter:blur(2px);backdrop-filter:blur(2px)}
.ov.show{display:block}

/* NAVBAR */
.nv{background:linear-gradient(135deg,var(--p),var(--pd));width:100%;position:relative;z-index:998}
.nv-in{max-width:1200px;margin:0 auto;padding:0 16px;display:flex;align-items:stretch;overflow-x:auto;scrollbar-width:none;-ms-overflow-style:none;-webkit-overflow-scrolling:touch}
.nv-in::-webkit-scrollbar{display:none}
.nl{color:rgba(255,255,255,.85);padding:12px 14px;font-size:12px;font-weight:600;white-space:nowrap;transition:var(--t);border-bottom:3px solid transparent;display:flex;align-items:center;gap:4px;flex-shrink:0;cursor:pointer}
...
.dd-arr{font-size:8px;transition:var(--t)}
.dd.open .dd-arr{transform:rotate(180deg)}
.dd-menu{position:absolute;top:100%;right:0;background:var(--w);min-width:260px;border-radius:0 0 var(--r) var(--r);box-shadow:var(--shl);display:none;z-index:1050;overflow:hidden;border:1px solid var(--g200);border-top:none}
.dd.open .dd-menu{display:block}
.dd-menu a{display:flex;align-items:center;gap:8px;padding:12px 16px;color:var(--d);font-size:13px;font-weight:600;border-bottom:1px solid var(--g100);transition:var(--t)}
.dd-menu a:last-child{border:none}
.dd-menu a:hover{background:var(--g50);color:var(--p);padding-right:20px}

/* MOBILE NAV */
.mob-nav{position:fixed;top:0;right:-100%;width:min(300px,85vw);height:100%;background:linear-gradient(180deg,var(--pd),var(--p) 60%,var(--pl));z-index:1001;transition:right .35s cubic-bezier(.4,0,.2,1);overflow-y:auto;overflow-x:hidden;padding:60px 0 30px;-webkit-overflow-scrolling:touch}
.mob-nav.open{right:0}
.mob-close{position:absolute;top:14px;left:14px;width:34px;height:34px;background:rgba(255,255,255,.15);border:none;border-radius:50%;color:#fff;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:var(--t)}
.mob-close:hover,.mob-close:focus{background:rgba(255,255,255,.3)}
.mob-link{display:flex;align-items:center;gap:8px;padding:13px 20px;color:rgba(255,255,255,.9);font-size:14px;font-weight:600;border-bottom:1px solid rgba(255,255,255,.08);transition:var(--t)}
.mob-link:hover{background:rgba(255,255,255,.1);color:#fff;padding-right:26px}
.mob-sec{padding:10px 20px 6px;color:rgba(255,255,255,.45);font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase}
.mob-sub{display:flex;align-items:center;gap:8px;padding:10px 30px;color:rgba(255,255,255,.8);font-size:13px;font-weight:600;border-bottom:1px solid rgba(255,255,255,.05);transition:var(--t)}
...
/* TICKER */
.ticker{background:linear-gradient(135deg,#e05000,#f7931e);padding:7px 0;overflow:hidden}
...
/* HERO */
.hero{background:linear-gradient(135deg,var(--pd),var(--p) 50%,var(--pl));color:#fff;padding:40px 16px;text-align:center;position:relative;overflow:hidden}
.hero::before{content:'';position:absolute;inset:0;background:url("data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%23fff' fill-opacity='.04'%3E%3Ccircle cx='20' cy='20' r='3'/%3E%3C/g%3E%3C/svg%3E");pointer-events:none}
...
.sr-b{display:flex;background:var(--w);border-radius:50px;box-shadow:0 8px 30px rgba(0,0,0,.15);overflow:hidden;border:3px solid transparent;transition:var(--t)}
.sr-b:focus-within{border-color:var(--p)}
.sr-b input{flex:1;border:none;padding:12px 18px;font-size:14px;font-family:'Cairo',sans-serif;outline:none;background:transparent;min-width:0;color:var(--d)}
.sr-b button{background:linear-gradient(135deg,var(--p),var(--pl));color:#fff;border:none;padding:12px 20px;cursor:pointer;font-family:'Cairo',sans-serif;font-weight:700;font-size:13px;white-space:nowrap;flex-shrink:0;transition:var(--t)}
.sr-b button:hover{opacity:.9}

/* AD */
.ad{background:var(--g100);border:2px dashed var(--g300);border-radius:var(--r);padding:12px;text-align:center;margin:16px 0;min-height:80px;display:flex;align-items:center;justify-content:center;color:var(--g400);font-size:11px;width:100%}
.adc{max-width:1200px;margin:16px auto 0;padding:0 16px}

/* LAYOUT */
.mg{max-width:1200px;margin:20px auto;padding:0 16px;display:grid;grid-template-columns:1fr 300px;gap:24px;align-items:start}

/* CATS */
.cb{display:flex;gap:8px;margin-bottom:16px;overflow-x:auto;padding-bottom:8px;-webkit-overflow-scrolling:touch;scrollbar-width:none;-ms-overflow-style:none}
.cb::-webkit-scrollbar{display:none}
.cc{display:flex;align-items:center;gap:5px;padding:7px 14px;border-radius:50px;font-size:11px;font-weight:700;white-space:nowrap;transition:var(--t);border:2px solid var(--g200);background:var(--w);color:var(--g600);flex-shrink:0;cursor:pointer}
.cc:hover,.cc.ac{border-color:var(--p);color:var(--p);background:rgba(15,76,129,.05);box-shadow:var(--sh)}

/* FEATURED */
.fp{display:block;background:linear-gradient(135deg,var(--p),var(--pd));border-radius:var(--rl);color:#fff;margin-bottom:20px;overflow:hidden;transition:var(--t);text-decoration:none}
.fp:hover{transform:translateY(-3px);box-shadow:var(--shl)}
.fp-img{width:100%;height:clamp(180px,30vw,240px);object-fit:cover}
...
.pc{display:block;background:var(--w);border-radius:var(--rl);overflow:hidden;box-shadow:var(--sh);transition:var(--t);border:1px solid var(--g100);text-decoration:none;color:inherit}
.pc:hover{transform:translateY(-3px);box-shadow:var(--shl);border-color:var(--p)}
.pc-img{width:100%;height:clamp(150px,25vw,200px);object-fit:cover;transition:transform .4s ease}
...
/* ARTICLE */
.ag{max-width:1200px;margin:20px auto;padding:0 16px;display:grid;grid-template-columns:1fr 300px;gap:24px;align-items:start}
.am{background:var(--w);border-radius:var(--rl);overflow:hidden;box-shadow:var(--sh)}
.ah-img{width:100%;height:clamp(180px,35vw,320px);object-fit:cover}
...
@media(max-width:480px){
  ...
  .mob-nav{width:100%}
  .ft-g{padding:20px 14px}
  .sc{padding:18px}
  .btt{bottom:14px;left:14px;width:38px;height:38px;font-size:16px}
}

/* PRINT */
@media print{.hd,.nv,.tb,.sb,.ft,.ad,.share,.btt,.sr-w,.adc,.mob-nav,.ov,.mbtn,.ticker{display:none!important}.mg,.ag{grid-template-columns:1fr}.am{box-shadow:none;border-radius:0}}
</style>`;
...
(function(){
var no=false;
function oNav(){no=true;document.getElementById('mn').classList.add('open');document.getElementById('ov').classList.add('show');document.getElementById('mb').classList.add('open');document.body.style.overflow='hidden';}
function cNav(){no=false;['mn','ov'].forEach(function(id){var e=document.getElementById(id);if(e)e.classList.remove('open','show');});var mb=document.getElementById('mb');if(mb)mb.classList.remove('open');document.body.style.overflow='';}
window.tNav=function(){no?cNav():oNav();};
window.cNav=cNav;
window.tDD=function(e){e.preventDefault();e.stopPropagation();var d=document.getElementById('ddd');if(d)d.classList.toggle('open');};
document.addEventListener('click',function(e){var d=document.getElementById('ddd');if(d&&!d.contains(e.target))d.classList.remove('open');});
window.addEventListener('resize',function(){if(window.innerWidth>768&&no)cNav();},{passive:true});
window.addEventListener('scroll',function(){var b=document.getElementById('btt');if(b)b.style.display=window.scrollY>350?'flex':'none';},{passive:true});
window.doSearch=function(){var i=document.getElementById('srch');if(!i)return;var q=i.value.toLowerCase().trim();document.querySelectorAll('.pc,.fp').forEach(function(el){el.style.display=q&&el.textContent.toLowerCase().indexOf(q)<0?'none':'';});};
})();
</script>`;
...
function mkHeader(base) {
  ...
  return `
<div class="tb"><div class="tb-in"><span>📅 ${today}</span><div><a href="${base}/about">من نحن</a><a href="${base}/contact">اتصل بنا</a></div></div></div>
...
<button class="mbtn" id="mb" onclick="tNav()" aria-label="فتح القائمة" aria-expanded="false"><i></i><i></i><i></i></button>
</div></header>
<div class="ov" id="ov" onclick="cNav()" role="presentation"></div>
<div class="mob-nav" id="mn" role="navigation" aria-label="القائمة الجانبية">
...
<nav class="nv" role="navigation" aria-label="القائمة الرئيسية"><div class="nv-in">
<a href="${base}/" class="nl ac">🏠 الرئيسية</a>
<div class="dd nl" id="ddd">
<div class="dd-btn nl" onclick="tDD(event)" role="button" aria-haspopup="true" aria-expanded="false">📂 الأقسام <span class="dd-arr">▼</span></div>
<div class="dd-menu" role="menu">
${CATS.map(c => `<a href="${base}/category/${encodeURIComponent(c.name)}" role="menuitem"><span class="cdot" style="background:${c.color}"></span>${c.icon} ${c.name}</a>`).join("")}
</div>
</div>
...
function mkFooter(base) {
  ...
  return `
<footer class="ft" role="contentinfo"><div class="ft-g">
<div>
<h3>🇲🇦 بوابة الخدمات الرقمية</h3>
<p>منصة إلكترونية مستقلة متخصصة في تبسيط الإجراءات الإدارية وتقديم معلومات الخدمات الحكومية الرقمية للمواطن المغربي بشكل مبسط وواضح.</p>
</div>
...
<li><a href="https://www.rsu.ma" target="_blank" rel="noopener noreferrer">السجل الاجتماعي RSU</a></li>
...
<div class="ft-b">
<p>© ${yr} بوابة الخدمات الرقمية | جميع الحقوق محفوظة</p>
...
function mkSidebar(idx, base) {
  ...
  const cats = [...new Set(idx.map(p => p.category))];
  return `<aside class="sb" role="complementary">
<div class="ad" aria-label="مساحة إعلانية"><!-- AdSense Sidebar Top --></div>
<div class="wg">
<h3 class="wt">📊 الأكثر قراءة</h3>
${recent.map((p, i) => `<a href="${base}/post/${p.slug}" class="ti">
<div class="tn">${i + 1}</div>
<div class="tt"><h4>${p.title.length > 52 ? p.title.substring(0, 52) + "…" : p.title}</h4><span>${p.date}${p.views ? " · 👁️ " + p.views : ""}</span></div>
</a>`).join("")}
</div>
<div class="wg">
<h3 class="wt">📂 التصنيفات</h3>
<ul class="ll">
${cats.map(c => { const n = idx.filter(p => p.category === c).length, cl = catColor(c), ic = catIcon(c); return `<li><a href="${base}/category/${encodeURIComponent(c)}"><span class="cdot" style="background:${cl}"></span>${ic} ${c} <small style="margin-right:auto;color:var(--g400)">(${n})</small></a></li>`; }).join("")}
</ul>
</div>
<div class="wg nwl">
<h3 class="wt">✉️ النشرة البريدية</h3>
<p>اشترك واحصل على آخر المقالات مباشرة في بريدك</p>
...
<div class="ad" aria-label="مساحة إعلانية"><!-- AdSense Sidebar Bottom --></div>
<div class="wg">
<h3 class="wt">🔗 روابط مفيدة</h3>
<ul class="ll">
<li><a href="https://www.rsu.ma" target="_blank" rel="noopener noreferrer">السجل الاجتماعي الموحد</a></li>
<li><a href="https://www.cnss.ma" target="_blank" rel="noopener noreferrer">CNSS - الضمان الاجتماعي</a></li>
<li><a href="https://www.ofppt.ma" target="_blank" rel="noopener noreferrer">OFPPT - التكوين المهني</a></li>
<li><a href="https://ae.gov.ma" target="_blank" rel="noopener noreferrer">بوابة المقاول الذاتي</a></li>
<li><a href="https://www.cnie.ma" target="_blank" rel="noopener noreferrer">البطاقة الوطنية CNIE</a></li>
</ul>
</div>
</aside>`;
}
...
function pgHome(idx, base) {
  ...
  const tvd = tv > 9999 ? Math.floor(tv / 1000) + "K+" : tv + "+";

  const schema = JSON.stringify({
    "@context": "https://schema.org", "@type": "WebSite",
    "name": "بوابة الخدمات الرقمية", "url": base,
...
<head>
${mkHead(
  "بوابة الخدمات الرقمية | دليل المواطن المغربي 2026",
  ...
  "خدمات رقمية المغرب, أمو تضامن, السجل الاجتماعي, مباريات التوظيف 2026, CNSS, OFPPT, جواز السفر, البطاقة الوطنية, المقاول الذاتي, فيزا شنغن, دعم السكن",
  base + "/",
  `<meta property="og:type" content="website"><meta property="og:title" content="بوابة الخدمات الرقمية"><meta property="og:description" content="دليلك الشامل للخدمات الحكومية الرقمية بالمغرب"><meta property="og:url" content="${base}/"><meta name="twitter:card" content="summary_large_image"><script type="application/ld+json">${schema}</script>`
)}
</head><body>
${mkHeader(base)}
${idx.length > 0 ? `<div class="ticker" aria-hidden="true"><div class="ticker-t">${idx.slice(0, 8).concat(idx.slice(0, 8)).map(p => `<span>📌 ${p.title.substring(0, 55)}…</span>`).join("")}</div></div>` : ""}
<section class="hero">
<div class="hero-c">
...
<div class="sr-w">
<div class="sr-b" role="search">
<input type="search" id="srch" placeholder="🔍 ابحث عن خدمة أو إجراء إداري..." onkeyup="doSearch()" aria-label="البحث في الموقع">
<button onclick="doSearch()">بحث</button>
</div>
</div>
<div class="adc"><div class="ad"><!-- Google AdSense - Leaderboard --></div></div>
<div class="mg">
<main role="main">
...
${feat ? `<a href="${base}/post/${feat.slug}" class="fp" aria-label="${esc(feat.title)}">
...
<div class="fp-m"><span>📅 ${feat.date}</span><span>⏱️ ${feat.readTime || 3} دقائق</span><span>👁️ ${feat.views || 0}</span></div>
</div></a>` : ""}
<div class="ad"><!-- Google AdSense - In-Article --></div>
<div class="pl">
${rest.length === 0 && !feat ? `<div class="empty"><div class="empty-icon">📚</div><h3>المحتوى قادم قريباً</h3><p>يتم إعداد المقالات والأدلة الإرشادية</p></div>` :
rest.map((p, i) => {
  const cl = p.categoryColor || catColor(p.category);
  const ic = p.categoryIcon || catIcon(p.category);
  return `<a href="${base}/post/${p.slug}" class="pc" aria-label="${esc(p.title)}">
...
<div class="pc-ft"><span>📅 ${p.date}</span><span>👁️ ${p.views || 0}</span><span class="rm">اقرأ المزيد ←</span></div>
</div></a>${(i + 1) % 3 === 0 ? '<div class="ad"><!-- AdSense In-Feed --></div>' : ""}`;
}).join("")}
</div>
</main>
${mkSidebar(idx, base)}
...
function pgArticle(post, idx, base) {
  ...
  const artSch = JSON.stringify({
    ...
    "articleSection": post.category, "inLanguage": "ar", "keywords": post.keywords,
    "wordCount": post.wordCount, ...(post.image ? { "image": base + "/image/" + post.slug } : {})
  });
...
<div class="ag">
<article class="am" itemscope itemtype="https://schema.org/Article" role="main">
${post.image ? `<img class="ah-img" src="${base}/image/${post.slug}" alt="${esc(post.imageAlt || post.title)}" width="1024" height="320" loading="eager" decoding="async" itemprop="image">` : ""}
<div class="ab">
<nav class="bc" aria-label="مسار التنقل">
<a href="${base}/">🏠 الرئيسية</a><sep>›</sep>
<a href="${base}/category/${encodeURIComponent(post.category)}">${ic} ${post.category}</a><sep>›</sep>
<span>${post.title.substring(0, 32)}…</span>
</nav>
<header class="ah">
...
<div class="am-meta">
<span>📅 <time itemprop="datePublished" datetime="${post.dateISO || ""}">${post.date}</time></span>
...
<div class="ac" itemprop="articleBody">${post.content}</div>
...
${rel.length ? `<section class="rel"><h3>📚 مقالات ذات صلة</h3><div class="rg">${rel.map(r => `<a href="${base}/post/${r.slug}" class="rc"><h4>${r.title}</h4><span>📅 ${r.date}</span></a>`).join("")}</div></section>` : ""}
</div>
</article>
${mkSidebar(idx, base)}
</div>
...
${mkHead(
  ...
  `<meta property="og:type" content="website"><meta property="og:title" content="${esc(catName)}"><meta property="og:url" content="${base}/category/${encodeURIComponent(catName)}"><script type="application/ld+json">${schema}</script>`
)}
</head><body>
${mkHeader(base)}
...
<div class="mg">
<main role="main">
<nav class="bc" style="margin-bottom:14px" aria-label="مسار التنقل">
<a href="${base}/">🏠 الرئيسية</a><sep>›</sep><span>${ic} ${catName}</span>
</nav>
<div class="ad"><!-- AdSense --></div>
<div class="pl">
${filtered.length === 0 ? `<div class="empty"><div class="empty-icon">${ic}</div><h3>لا توجد مقالات بعد</h3><p><a href="${base}/" style="color:var(--p)">← العودة للرئيسية</a></p></div>` :
filtered.map((p, i) => {
...
${p.hasImage ? `<img class="pc-img" src="${base}/image/${p.slug}" alt="${esc(p.imageAlt || p.title)}" width="600" height="200" loading="lazy" decoding="async">` : ""}
<div class="pc-b">
<div class="pc-top"><span class="bdg" style="background:${pCl}18;color:${pCl}"><span class="cdot" style="background:${pCl}"></span>${pIc} ${p.category}</span></div>
<h3>${p.title}</h3>
<p class="pc-ex">${p.description || ""}</p>
<div class="pc-ft"><span>📅 ${p.date}</span><span>👁️ ${p.views || 0}</span><span class="rm">اقرأ المزيد ←</span></div>
</div></a>${(i + 1) % 3 === 0 ? '<div class="ad"><!-- AdSense --></div>' : ""}`;
}).join("")}
</div>
</main>
${mkSidebar(idx, base)}
...
${mkHead(
  ...
  "صفحة " + title + " - بوابة الخدمات الرقمية المغربية",
  title + ", بوابة الخدمات الرقمية",
  base + "/" + slug,
...
<p>تلتزم <strong>بوابة الخدمات الرقمية</strong> بحماية خصوصية مستخدميها. تشرح هذه السياسة كيفية جمع البيانات واستخدامها.</p>
...
<li><strong>الاشتراك في النشرة:</strong> البريد الإلكتروني فقط</li>
</ul>
<h2>Google AdSense والإعلانات</h2>
<p>يستخدم موقعنا <strong>Google AdSense</strong> لعرض إعلانات ملائمة. تستخدم Google ملفات تعريف الارتباط لعرض إعلانات بناءً على زياراتك السابقة. يمكنك إلغاء الإعلانات المخصصة من <a href="https://www.google.com/settings/ads" target="_blank" rel="noopener noreferrer">إعدادات إعلانات Google</a>.</p>
<h2>Google Analytics</h2>
<p>نستخدم Google Analytics لفهم كيفية استخدام الزوار للموقع. البيانات مجهولة الهوية ولا تُحدد هويتك الشخصية.</p>
<h2>ملفات تعريف الارتباط</h2>
<p>نستخدم الكوكيز لـ: تحسين الأداء، تحليل الزيارات، وعرض إعلانات ملائمة. يمكنك تعطيلها من إعدادات متصفحك.</p>
<h2>مشاركة البيانات</h2>
<p>لا نبيع بياناتك. نشارك معلومات مجهولة فقط مع: Google (AdSense وAnalytics) وCloudflare (استضافة).</p>
<h2>حقوقك</h2>
<ul>
<li>الوصول إلى بياناتك الشخصية</li>
<li>طلب تصحيح أو حذف بياناتك</li>
<li>إلغاء الاشتراك في النشرة في أي وقت</li>
</ul>
<h2>التغييرات</h2>
<p>نحتفظ بحق تعديل هذه السياسة مع تحديث تاريخ "آخر تحديث" أعلى الصفحة.</p>
<h2>اتصل بنا</h2>
...
<ul>
<li>يُحظر استخدام الموقع لأغراض غير قانونية</li>
<li>يُحظر محاولة اختراق أو تعطيل الموقع</li>
<li>يُحظر جمع البيانات بشكل آلي دون إذن</li>
</ul>
<h2>4. الإعلانات</h2>
<p>يحتوي الموقع على إعلانات Google AdSense. نحن غير مسؤولين عن محتوى الإعلانات.</p>
<h2>5. تحديد المسؤولية</h2>
<p>لا نتحمل مسؤولية أي ضرر ناتج عن استخدام المعلومات المنشورة. المستخدم مسؤول عن قراراته.</p>
...
<h1>🇲🇦 من نحن</h1>
<h2>رؤيتنا</h2>
<p><strong>بوابة الخدمات الرقمية</strong> منصة إعلامية مستقلة تهدف إلى تبسيط الإجراءات الإدارية والخدمات الحكومية الرقمية للمواطن المغربي، وتقديم معلومات دقيقة وموثوقة بأسلوب واضح ومبسط.</p>
<h2>ما نقدمه</h2>
<ul>
<li>📝 أدلة مفصلة للإجراءات الإدارية الرقمية</li>
<li>📢 تغطية المستجدات والأخبار المتعلقة بالخدمات الحكومية</li>
...
<p><strong>آخر تحديث:</strong> ${d}</p>
<p>نحترم حقوق الملكية الفكرية ونستجيب لإشعارات انتهاك حقوق النشر وفق قانون DMCA.</p>
...
<li>إخطار الطرف المعني بالإزالة</li>
</ul>
<h2>كيفية الإرسال</h2>
...
<p><strong>آخر تحديث:</strong> ${d}</p>
<h2>طبيعة المحتوى</h2>
<p>جميع المعلومات المنشورة على <strong>بوابة الخدمات الرقمية</strong> هي لأغراض <strong>إعلامية وتثقيفية فقط</strong>. نبذل جهدنا لتقديم معلومات دقيقة لكن لا نضمن صحتها أو اكتمالها في جميع الأوقات.</p>
<h2>ليس استشارة رسمية</h2>
<p>المحتوى <strong>لا يُعد بديلاً</strong> عن:</p>
<ul>
<li>الاستشارة القانونية المتخصصة</li>
<li>المعلومات الرسمية من الجهات الحكومية المختصة</li>
...
<h2>الإعلانات</h2>
<p>الإعلانات المعروضة عبر Google AdSense لا تعكس توصياتنا ولسنا مسؤولين عن محتواها.</p>
<h2>الاستقلالية</h2>
<p>موقعنا مستقل تماماً وغير تابع لأي جهة حكومية. المحتوى يعكس آراء فريق التحرير فقط.</p>
<h2>موافقتك</h2>
