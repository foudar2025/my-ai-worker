export default {
  ...
  async fetch(request, env) {
    ...
    const getIdx = async () => {
      try { return await env.BLOG_KV.get("posts_index", { type: "json" }) || []; }
      catch (e) { return []; }
    };
    const getPost = async (slug) => {
      try { return await env.BLOG_KV.get("post:" + slug, { type: "json" }); }
      catch (e) { return null; }
    };
    ...
    const X = (xml) => new Response(xml, {
      headers: { "Content-Type": "application/xml;charset=UTF-8", "Cache-Control": "public,max-age=3600" }
    });
    const T = (txt) => new Response(txt, {
      headers: { "Content-Type": "text/plain;charset=UTF-8", "Cache-Control": "public,max-age=86400" }
    });

    if (path === "/sitemap.xml") {
      const idx = await getIdx();
      return X(mkSitemap(idx, base));
    }
    if (path === "/robots.txt") {
    ...
    if (path === "/feed.xml" || path === "/rss.xml") {
      const idx = await getIdx();
      return X(mkRSS(idx, base));
    }

    const staticMap = {
      "/privacy": pgPrivacy, "/terms": pgTerms, "/contact": pgContact,
      "/about": pgAbout, "/dmca": pgDMCA, "/disclaimer": pgDisclaimer
    };
    if (staticMap[path]) {
      const idx = await getIdx();
      return H(staticMap[path](base, idx), 86400);
    }
...
.mbtn{display:none;background:var(--w);border:2px solid var(--g300);cursor:pointer;border-radius:8px;width:42px;height:42px;min-width:42px;flex-direction:column;align-items:center;justify-content:center;gap:5px;flex-shrink:0;padding:0;position:relative;z-index:502}
.mbtn i{display:block;width:18px;height:2px;background:var(--d);border-radius:2px;transition:var(--t);pointer-events:none}
...
.ov{display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:498;-webkit-backdrop-filter:blur(2px);backdrop-filter:blur(2px)}
...
.nl{color:rgba(255,255,255,.85);padding:12px 14px;font-size:12px;font-weight:600;white-space:nowrap;transition:var(--t);border-bottom:3px solid transparent;display:flex;align-items:center;gap:5px;flex-shrink:0;cursor:pointer}
...
.dd-menu{position:absolute;top:100%;right:0;background:var(--w);min-width:260px;border-radius:0 0 var(--r) var(--r);box-shadow:0 8px 30px rgba(0,0,0,.15);display:none;z-index:9999;overflow:hidden;border:1px solid var(--g200);border-top:none}
...
.mob-nav{position:fixed;top:0;right:-100%;width:min(300px,85vw);height:100%;background:linear-gradient(180deg,var(--pd),var(--p) 60%,var(--pl));z-index:500;transition:right .35s cubic-bezier(.4,0,.2,1);overflow-y:auto;overflow-x:hidden;padding:60px 0 30px;-webkit-overflow-scrolling:touch}
...
.mob-sec{padding:10px 20px 6px;color:rgba(255,255,255,.45);font-size:10px;font-weight:700;letter-spacing:1px}
...
.hero{background:linear-gradient(135deg,var(--pd),var(--p) 50%,var(--pl));color:#fff;padding:40px 16px;text-align:center;position:relative;overflow:hidden;width:100%}
...
.sr-b input{flex:1;border:none;padding:13px 18px;font-size:14px;font-family:'Cairo',sans-serif;outline:none;background:transparent;min-width:0;color:var(--d)}
.sr-b button{background:linear-gradient(135deg,var(--p),var(--pl));color:#fff;border:none;padding:13px 22px;cursor:pointer;font-family:'Cairo',sans-serif;font-weight:700;font-size:13px;white-space:nowrap;flex-shrink:0;transition:var(--t)}
...
.mg{max-width:1200px;margin:24px auto;padding:0 16px;display:grid;grid-template-columns:1fr 300px;gap:24px;align-items:start}
...
.pl{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:18px;align-items:start}
.pc{display:block;background:var(--w);border-radius:var(--rl);overflow:hidden;box-shadow:var(--sh);transition:var(--t);border:1px solid var(--g100);text-decoration:none;color:inherit;height:fit-content}
